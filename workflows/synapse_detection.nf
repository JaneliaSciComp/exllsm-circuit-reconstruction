...
workflow find_synapses_without_neuron_info {
    take:
    dataset
    synapse_stack_dir
    output_dir
    ...
    | map {
        ...
        [
            "${synapse_h5_file.name}", // synapse_h5_file_name
    ...
    | map {
        ...
        [
            ...
            it[1], // synapse_h5_file for postprocessing
        ]
    }

    def synapse_mask_inputs = synapse_seg_copy
    | join(synapse_seg_results, by:0)
    | flatMap {
        // [ working_dir, synapse_h5_file, synapse_h5_file_name, synapse_vol_partitions, index, output_dir ]
        def synapse_h5_file = it[1]
        it[3].collect {
            [
                synapse_h5_file,
                it, // volume limits
    ...
    def masked_synapses = mask_synapses(
        synapse_mask_inputs.map { it[0] }, // synapse h5 image
        '', // neuron mask image
        synapse_mask_inputs.map { it[1] }, // synapse vol partition
    ...
    emit:
    done = masked_synapses
}

workflow find_synapses_with_neuron_info {
    take:
    dataset
    synapse_stack_dir
    neuron1_stack_dir
    ...
    // copy synapse segmentation for masking it with neuron1 image
    def synapse_seg_copy = cp_file (
        synapse_seg_results.map {
            [
    ...
    if (neuron2_stack_dir) {
        ...
        masked_synapses = neuron2_masked_synapses
    } else {
        masked_synapses = neuron1_masked_synapses
    }

    def masked_synapses_as_tiff_inputs = masked_synapses
    ...
    def masked_synapses_as_tiff = hdf5_to_tiff(
        masked_synapses_as_tiff_inputs.map { it[0] },
        masked_synapses_as_tiff_inputs.map { "${it[1]}/masked_synapses_tiff_results" }
    )

    emit:
    done = masked_synapses_as_tiff
}
