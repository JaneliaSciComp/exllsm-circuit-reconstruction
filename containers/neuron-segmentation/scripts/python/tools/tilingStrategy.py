...
class UnetTiling3D(Tiling):
    ...
    def getInputVolume(self):
        ...
        """
        start_corner = self.getInputTile(
            0)[:3]  # get the first corner of the first input tile
...
class Canvas():
    ...
    def cropAndPadAABB(self, aabb):
        ...
        post_pad = [np.max([0, aabb[i+3] - self.image.shape[i]])
                    for i in range(3)]
...
class AbsoluteCanvas():
    def __init__(self, image_shape: tuple, canvas_area: tuple, image: np.array):
        ...
        for d in range(3):
            assert canvas_area[d+3] - canvas_area[d] == image.shape[d], 'The image must have the same extent as the canvas area' + \
                '\nfor d = {} canvas area was {} image shape was {}'.format(
                    d, canvas_area[d+3] - canvas_area[d], image.shape[d])
    ...
    def cropAndPadAABB(self, aabb):
        ...
        start = np.array([np.max([self.canvas_area[i], aabb[i]])
                          for i in range(3)])  # origo is at canvas_area[:3]
        stop = np.array([np.min([self.canvas_area[i+3], aabb[i+3]])
                         for i in range(3)])  # end is at canvas_area[3:]
        # calculate the padding in each direction
        pre_pad = np.array(
            [np.max([0, self.canvas_area[i] - aabb[i]]) for i in range(3)])
        post_pad = np.array(
            [np.max([0, aabb[i+3] - self.canvas_area[i+3]]) for i in range(3)])
        padding = tuple([(pre_pad[i], post_pad[i]) for i in range(3)])
...
class UnetTiler3D():
    ...
    def forEntireCongruentData(cls, image, mask=None, output_shape=(132, 132, 132), input_shape=(220, 220, 220)):
        ...
        if mask is None:
            # Allocate a tensor where the segmentation mask is stored
            mask_canvas = Canvas(np.zeros_like(image))
        else:
        ...
        tiling = UnetTiling3D(image_shape=image.shape, tiling_subvolume=None,
    ...
    def writeSlice(self, index, tile):
        ...
        # retrieve aabb that belongs to the slice index
        aabb = self.tiling.getOutputTile(index)
